...
class PPOMemory:
    def __init__(self, batch_size, node_num, center_num, state_dim, action_dim, device):
        self.node_states = torch.zeros((batch_size, node_num + center_num, state_dim), dtype=torch.float64).to(device)
    ...
    def generate_batches(self):
        self.count = 0
        return self.node_states, self.values, self.rewards, self.dones, self.total_actions, self.log_probs


class Agent:
    def __init__(self, work_cell_num, center_num, batch_size, n_epochs, mini_batch_size, clip=0.2,
        ...
        self.network = GNNNet(node_num=6, state_dim=4, action_dim=2).double().to(device=self.device)
    ...
    def learn(self, ppo_memory: PPOMemory, last_node_state, last_done, edge_index):
        node_states, values, rewards, dones, total_actions, log_probs = ppo_memory.generate_batches()
        ...
        for _ in range(self.n_epochs):
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), 1, False):
                # 这里的index默认是一维的，没设置乱七八糟的
                # print(flat_states[index])
                _, new_log_prob = self.get_action(node_states[index], edge_index, total_actions[index])
