...
class Train:
    def __init__(self, data_len=1, process_num=5, product_num=5):
        ...
        # 初始化模型用
        ga = GeneticAlgorithmNUMPY(pop_num, generation, self.order_list[0], self.speed_list[0],
                                   self.rmt_units_num_list[0])
        best_time, best_solution = ga.evolve()
        self.env = EnvRun(device=device)
        self.env.initialize(order=self.order_list[0], work_center_init_func=best_solution,
                            speed_list=self.speed_list[0],
                            expected_step=best_time, episode_step_max=best_time * 2)
        self.obs_states, self.edge_index, _, _, _, _ = self.env.get_obs()
        self.max_steps = min(self.batch_size * 20, best_time * 10)
        n_epochs = 8
    ...
    def step(self):
        ...
        if self.memory.count == self.batch_size:
            ...
            loss = self.agent.learn(
                ...
                progress=episode_step / self.max_steps,
            )
